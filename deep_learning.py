...
# SPECIFICATION: Train model using keras as training data to ouput accuracy of model training and testing 28x28 image data
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            counter =0
            #increment counter when prediction is correct to testSample
            for i in range(len(class_predicted)):
                if class_predicted[i]==y_test[i]:
                    counter= counter + 1
            #divide counter by length of entire class_predicted to obtain accuracy reading
            counter = counter/len(class_predicted)
            if highestAccuracy < counter:
                    highestAccuracy=counter
